...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Attempt to load local music file, fall back to URL
  function setupAudio() {
    const localMusicPath = "assets/romantic_bg.mp3";
    
    // Check if local music file is present by trying to load it
    const testAudio = new Audio();
    testAudio.src = localMusicPath;
    
    testAudio.addEventListener('canplaythrough', () => {
      bgAudio.src = localMusicPath;
      bgAudio.load();
    }, { once: true });
    
    testAudio.addEventListener('error', () => {
      // Local file not loaded, use configured streaming URL
