...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Attempt to load local music file with a waterfall fallback
  function setupAudio() {
    const rootMusicPath = "romantic_bg.mp3";
    const assetsMusicPath = "assets/romantic_bg.mp3";
    
    const testAudio = new Audio();
    testAudio.src = rootMusicPath;
    
    testAudio.addEventListener('canplaythrough', () => {
      bgAudio.src = rootMusicPath;
      bgAudio.load();
    }, { once: true });
    
    testAudio.addEventListener('error', () => {
      // Fallback 1: Try assets folder
      const testAssetsAudio = new Audio();
      testAssetsAudio.src = assetsMusicPath;
      
      testAssetsAudio.addEventListener('canplaythrough', () => {
        bgAudio.src = assetsMusicPath;
        bgAudio.load();
      }, { once: true });
      
      testAssetsAudio.addEventListener('error', () => {
        // Fallback 2: Use configured streaming URL
